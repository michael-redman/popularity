...
int main(int argc, char ** argv){
	...
	PGconn *pg_conn;
	PGresult *pg_result;
	double popularity, avg_votes, avg_time_since_last_vote, avg_length;
	long pool_count;
	...
	if	(PQstatus(pg_conn)!=CONNECTION_OK)
		{	exit_status|=1;
			SQLERR;
			AT; goto label0; }
	pg_result=PQexec(pg_conn,"select count(*), avg(votes), avg(extract(epoch from now()-time)), avg(duration) from pool where path is not null");
	...
	if	(PQresultStatus(pg_result)!=PGRES_COMMAND_OK)
		{	exit_status|=1; SQLERR; AT;
			PQclear(pg_result); goto label1; }
	PQclear(pg_result);
	pg_result=PQexec(pg_conn,"DECLARE foo CURSOR FOR select hash, duration, votes, extract(epoch from now()-time), path from pool where path is not null;");
	if	(PQresultStatus(pg_result)!=PGRES_COMMAND_OK)
		{	exit_status|=1; SQLERR; AT;
	...
	node.cumul_density+=r_to_rplus(popularity*length+avg_votes*avg_length*avg_time_since_last_vote/file_age_v);
